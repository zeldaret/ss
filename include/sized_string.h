 ...
 */
template <size_t Size> struct SizedString {
  SizedString() { mChars[0] = '\0'; }

  SizedString(const char *src) { operator=(src); }

  char mChars[Size];

  operator char *() { return mChars; }
  ...
  bool operator==(const char *other) const { return strequals(mChars, other); }

...
void ScnObj::CalcWorldMtx(const math::MTX34 *pParent, u32 *pParam) {
    ...
    if (pParent != NULL) {
        if (TestScnObjFlag(SCNOBJFLAG_MTX_LOCAL_IDENTITY)) {
            ...
            math::MTX34Mult(&mMtxArray[MTX_WORLD], pParent, &mMtxArray[MTX_LOCAL]);
        }
    } else {
...
bool ScnObj::SetMtx(ScnObjMtxType type, const math::MTX34 &rMtx) {
    if (static_cast<u32>(type) < MTX_TYPE_MAX) {
        if (type == MTX_LOCAL) {
            SetScnObjFlag(SCNOBJFLAG_MTX_LOCAL_IDENTITY, FALSE);
        }

        math::MTX34Copy(&mMtxArray[type], &rMtx);
...
bool ScnObj::SetBoundingVolume(ScnObjBoundingVolumeType type, const math::AABB *pAABB) {
    if (pAABB != NULL) {
...
bool ScnObj::GetBoundingVolume(ScnObjBoundingVolumeType type, math::AABB *pAABB) const {
    if (pAABB != NULL) {
        if (type < BOUNDINGVOLUME_MAX) {
            *pAABB = mAABB[type];
            return true;
 ...
 *
 ******************************************************************************/
ScnObj::ForEachResult ScnLeaf::ForEach(ForEachFunc pFunc, void *pInfo, bool postOrder) {
#pragma unused(postOrder)

    return pFunc(this, pInfo) != FOREACHRESULT_CONTINUE ? FOREACHRESULT_OK : FOREACHRESULT_CONTINUE;
}
 ...
 *
 ******************************************************************************/
ScnObj::ForEachResult ScnGroup::ForEach(ForEachFunc pFunc, void *pInfo, bool postOrder) {
    ...
    if (postOrder) {
        ...
        return result == FOREACHRESULT_CONTINUE ? FOREACHRESULT_CONTINUE : FOREACHRESULT_OK;
    } else {
        result = pFunc(this, pInfo);
...
void ScnGroup::ScnGroup_G3DPROC_GATHER_SCNOBJ(u32 param, IScnObjGather *pCollection) {
    IScnObjGather::CullingStatus status = pCollection->Add(
...
void ScnGroup::ScnGroup_G3DPROC_CALC_WORLD(u32 param, const math::MTX34 *pParent) {
...
void ScnGroup::ScnGroup_G3DPROC_CALC_VIEW(u32 param, const math::MTX34 *pCamera) {
    ...
    for (u32 i = 0; i < mNumScnObj; i++) {
        mpScnObjArray[i]->G3dProc(G3DPROC_CALC_VIEW, param, const_cast<math::MTX34 *>(pCamera));
    }
...
void ScnGroup::DefG3dProcScnGroup(u32 task, u32 param, void *pInfo) {
    switch (task) {
        case G3DPROC_GATHER_SCNOBJ: {
            ScnGroup_G3DPROC_GATHER_SCNOBJ(param, static_cast<IScnObjGather *>(pInfo));
            break;
        }
...
ScnGroup::ScnGroup(MEMAllocator *pAllocator, ScnObj **ppObj, u32 capacity)
    : ScnObj(pAllocator), mpScnObjArray(ppObj), mSizeScnObj(capacity), mNumScnObj(0) {

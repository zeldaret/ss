...
void dSndSeSound_c::shiftPair(const UnkSeSoundStruct *pUnk) {
    if (pUnk == nullptr) {
        return;
    }
...
void dSndSeSound_c::shiftValue(f32 value) {
    for (int i = 9; i > 0; i--) {
        mpUnk[i].field_0x04 = mpUnk[i - 1].field_0x04;
    }
    mpUnk[0].field_0x04 = value;
}

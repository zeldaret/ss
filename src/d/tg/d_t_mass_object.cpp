...
bool dTgMassObjInstance::checkForHit(
    ...
    if (hitObj != nullptr) {
        ...
        if (hitObj->ChkAtType(AT_TYPE_BELLOWS)) {
            ...
            if ((s32)link->getSpecificAttackDirection() == CUT_DIR_U ||
                (s32)link->getSpecificAttackDirection() == CUT_DIR_D ||
                (s32)link->getSpecificAttackDirection() == CUT_DIR_STAB) {
                if (distance < fVar20) {
                    isNotCut = 0;
                } else {
                    maybeMaxImpactDistance = 300.f;
                ...
                if (mMassObjSubtype == 8 || mMassObjSubtype == 9) {
                    ...
                    distance = mGroundHeight.distance(hitPosition);
                    if ((s32)link->getSpecificAttackDirection() != CUT_DIR_LU &&
                        (s32)link->getSpecificAttackDirection() != CUT_DIR_RU) {
                        return true;
                    }
                    if (distance >= fVar21) {
                        return true;
...
// non matching
bool dTgMassObjInstance::FUN_80278c70(u32 param_2, dCcMassS_HitInf &param_3, dAcBase_c *param_4, GrassModel *param_5) {
